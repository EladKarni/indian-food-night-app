...
async function uploadMenuItems() {
  ...
  if (!supabaseUrl || !supabaseAnonKey) {
    console.error(
      "Supabase environment variables not set. Please check NEXT_PUBLIC_SUPABASE_URL and NEXT_PUBLIC_SUPABASE_ANON_KEY",
    );
    return;
  }
  ...
  menuData.categories.forEach((category: any) => {
    console.log(
      `Processing category: ${category.name} (${category.items.length} items)`,
    );
    ...
    category.items.forEach((item: any) => {
      menuItemsForDB.push({
        ...
        restaurant_name: menuData.restaurant,
      });
    });
  });
  ...
  try {
    ...
    for (let i = 0; i < batches.length; i++) {
      ...
      console.log(
        `Uploading batch ${i + 1}/${batches.length} (${batch.length} items)...`,
      );
      ...
      console.log(
        `Batch ${i + 1} uploaded successfully: ${data?.length} items inserted`,
      );
    }
    ...
    if (verifyError) {
      ...
      console.log(
        `✅ Verification: ${verifyData?.length} items found in database`,
      );
